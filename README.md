...
The prompt is the `simple` theme everywhere. In an ssh session it also puts the
hostname on the right, so a remote shell is obvious and, on student.cs, so is
which node the load balancer picked.
